...
import { getPostById, updatePost, uploadBlogImage } from "@/lib/supabase";
...
/** Simple HTML-to-text section extractor for existing posts. */
function parseExistingContent(html: string) {
  const result = { intro: "", doctorTip: "", mainBody: "", keyPoints: "", whenToConsult: "" };
  if (!html) return result;

  // Extract doctor tip
  const tipMatch = html.match(
    /<div class="clinical-callout"[^>]*>[\s\S]*?<strong>.*?<\/strong>\s*([\s\S]*?)\s*<\/div>/i
  );
  if (tipMatch) {
    result.doctorTip = tipMatch[1].replace(/<[^>]+>/g, "").trim();
  }

  // Split by <h2> sections
  const sections = html.split(/<h2>([\s\S]*?)<\/h2>/gi);
  for (let i = 1; i < sections.length; i += 2) {
    const heading = sections[i]?.replace(/<[^>]+>/g, "").trim().toLowerCase() || "";
    const body = sections[i + 1] || "";

    const plainText = body
      .replace(/<div class="clinical-callout"[\s\S]*?<\/div>/gi, "")
      .replace(/<li>/gi, "")
      .replace(/<\/li>/gi, "\n")
      .replace(/<[^>]+>/g, "")
      .trim();

    if (heading.includes("introduction") || heading.includes("what")) {
      result.intro = plainText;
    } else if (heading.includes("key") || heading.includes("factor") || heading.includes("common")) {
      result.keyPoints = plainText;
    } else if (heading.includes("when") || heading.includes("consult") || heading.includes("contact")) {
      result.whenToConsult = plainText;
    } else if (!heading.includes("doctor")) {
    ...
    async function load() {
      ...
      try {
        ...
        if (found) {
          ...
          setCategory(found.category as typeof CATEGORIES[number]);
          setCover(found.cover_image || COVERS[0].url);

          const parsed = parseExistingContent(found.content);
          setIntro(parsed.intro);
          setDoctorTip(parsed.doctorTip);
          setMainBody(parsed.mainBody);
          setKeyPoints(parsed.keyPoints);
          setWhenToConsult(parsed.whenToConsult);
        } else {
      ...
      {tab === "write" ? (
        <div className="space-y-5">
          ...
          <div className="bg-white rounded-3xl p-6 sm:p-8 border border-[#CFC3CC]/40 shadow-sm space-y-6">
            ...
            <span className={fieldHint}>A short summary — 2–3 sentences.</span>
            <textarea value={intro} onChange={(e) => setIntro(e.target.value)} rows={3} placeholder="Briefly describe what this article covers…" className={textareaClass} />
          </div>
          ...
          <div className="bg-white rounded-3xl p-6 sm:p-8 border border-l-4 border-[#CFC3CC]/40 border-l-[#D46789] shadow-sm space-y-3">
            <div className="flex items-center gap-2 text-[#D46789]">
              <span className="w-6 h-6 rounded-full bg-[#D46789] text-white text-xs font-bold flex items-center justify-center">4</span>
              <span className="text-sm font-bold">Doctor&apos;s Clinical Advice</span>
              <span className="text-[11px] font-semibold text-[#878787] ml-auto">Optional</span>
            </div>
            <span className={fieldHint}>A highlighted tip box with your key clinical recommendation.</span>
            <textarea value={doctorTip} onChange={(e) => setDoctorTip(e.target.value)} rows={2} placeholder="e.g. Early risk assessment during the first trimester…" className={`${textareaClass} !border-l-2 !border-l-[#D46789]/30`} />
          </div>

          <div className="bg-white rounded-3xl p-6 sm:p-8 border border-[#CFC3CC]/40 shadow-sm space-y-3">
            <div className="flex items-center gap-2 text-[#7B5A7E]">
              <span className="w-6 h-6 rounded-full bg-[#7B5A7E] text-white text-xs font-bold flex items-center justify-center">5</span>
              <span className="text-sm font-bold">Main Content</span>
            </div>
            <span className={fieldHint}>Write your detailed article text. Separate paragraphs with a blank line.</span>
            <textarea value={mainBody} onChange={(e) => setMainBody(e.target.value)} rows={8} placeholder="Write the detailed content here…" className={textareaClass} />
          </div>

          <div className="bg-white rounded-3xl p-6 sm:p-8 border border-[#CFC3CC]/40 shadow-sm space-y-3">
            <div className="flex items-center gap-2 text-[#2E7D32]">
              <span className="w-6 h-6 rounded-full bg-[#2E7D32] text-white text-xs font-bold flex items-center justify-center">6</span>
              <span className="text-sm font-bold">Key Points</span>
              <span className="text-[11px] font-semibold text-[#878787] ml-auto">Optional</span>
            </div>
            <span className={fieldHint}>One point per line — becomes a bullet list.</span>
            <textarea value={keyPoints} onChange={(e) => setKeyPoints(e.target.value)} rows={5} placeholder={"First point\nSecond point\nThird point"} className={textareaClass} />
          </div>

          <div className="bg-white rounded-3xl p-6 sm:p-8 border border-[#CFC3CC]/40 shadow-sm space-y-3">
            <div className="flex items-center gap-2 text-[#7B5A7E]">
              <span className="w-6 h-6 rounded-full bg-[#7B5A7E] text-white text-xs font-bold flex items-center justify-center">7</span>
              <span className="text-sm font-bold">When to See the Doctor</span>
              <span className="text-[11px] font-semibold text-[#878787] ml-auto">Optional</span>
            </div>
            <span className={fieldHint}>When patients should seek medical help.</span>
            <textarea value={whenToConsult} onChange={(e) => setWhenToConsult(e.target.value)} rows={2} placeholder="e.g. Seek urgent evaluation if you experience…" className={textareaClass} />
          </div>
        ...
        <div className="bg-white rounded-3xl p-6 sm:p-12 border border-[#CFC3CC]/40 shadow-sm space-y-8">
          <div className="flex flex-wrap items-center gap-3 text-xs">
            <span className="px-3.5 py-1 rounded-full bg-[#7B5A7E]/10 text-[#7B5A7E] font-bold uppercase">{category}</span>
            <span className="text-[#878787]">{new Date().toLocaleDateString("en-IN", { month: "long", day: "numeric", year: "numeric" })}</span>
            <span className="text-[#878787]">·</span>
            <span className="text-[#878787] font-medium">{readTime}</span>
          </div>

          <h1 className="text-3xl sm:text-4xl font-serif-display font-bold text-[#4E3953] leading-tight">{title || "Untitled Article"}</h1>
          ...
          {intro.trim() && (
            <p className="text-base text-[#464647] font-light leading-relaxed border-l-2 border-[#D46789] pl-4 italic bg-[#FAF7F9] py-3 rounded-r-xl">
              {intro.trim().length > 160 ? intro.trim().slice(0, 160) + "…" : intro.trim()}
            </p>
          )}
          ...
          {intro.trim() && (
            <div>
              <h2 className="text-xl sm:text-2xl font-serif-display font-bold text-[#4E3953] mb-3">Introduction</h2>
              <p className="text-base text-[#464647] leading-relaxed font-light">{intro}</p>
            </div>
          )}
          ...
          {mainBody.trim() && (
            <div>
              <h2 className="text-xl sm:text-2xl font-serif-display font-bold text-[#4E3953] mb-3">Detailed Information</h2>
              {mainBody.split(/\n{2,}/).filter(Boolean).map((p, i) => (
                <p key={i} className="text-base text-[#464647] leading-relaxed font-light mb-3">{p.trim()}</p>
              ))}
            </div>
          )}
          ...
          {keyPoints.trim() && (
            <div>
              <h2 className="text-xl sm:text-2xl font-serif-display font-bold text-[#4E3953] mb-3">Key Points</h2>
          ...
          {whenToConsult.trim() && (
            <div>
              <h2 className="text-xl sm:text-2xl font-serif-display font-bold text-[#4E3953] mb-3">When to See the Doctor</h2>
              <p className="text-base text-[#464647] leading-relaxed font-light">{whenToConsult}</p>
            </div>
          )}

          <div className="pt-8 border-t border-[#CFC3CC]/30 flex items-center gap-4">
            <div className="w-12 h-12 rounded-full bg-[#7B5A7E] text-white flex items-center justify-center font-bold text-sm shadow-sm">PW</div>
            <div>
              <p className="text-sm font-bold text-[#4E3953]">Dr. Pooja Wadgaonkar Patil</p>
              <p className="text-xs text-[#878787]">Consultant Gynaecologist &amp; Obstetrician</p>
            </div>
          </div>
        </div>
      )}
